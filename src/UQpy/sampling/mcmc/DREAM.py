...
class DREAM(MCMC):
    ...
    def run_one_iteration(self, current_state: np.ndarray, current_log_pdf: np.ndarray):
        ...
        """
        r_diff = np.array(
            [np.setdiff1d(np.arange(self.n_chains), j) for j in range(self.n_chains)]
        )
        cross = (
        ...
        unif_rvs = (
            Uniform()
            .rvs(
                nsamples=self.n_chains * (self.n_chains - 1),
        ...
        z = (
            Uniform()
            .rvs(
                nsamples=self.n_chains * self.dimension,
                random_state=self.random_state,
            )
        ...
        norm_vars = (
            ...
            .rvs(nsamples=self.n_chains**2, random_state=self.random_state)
    ...
    def check_outlier_chains(self, replace_with_best: bool = False):
        ...
        ind1, ind3 = (
            1 + round(0.25 * self.n_chains),
            1 + round(0.75 * self.n_chains),
        )
