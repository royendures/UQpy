...
def test_rect_rss():
    ...
    """
    marginals = [Uniform(loc=0.0, scale=2.0), Uniform(loc=0.0, scale=1.0)]
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [0.417022, 0.36016225],
                [1.00011437, 0.15116629],
                [0.14675589, 0.5461693],
                [1.18626021, 0.67278036],
                [1.90711287, 0.04595797],
                [0.80005026, 0.86428026],
            ]
        ),
    )
    assert np.allclose(
        np.array(y.samplesU01),
        np.array(
            [
                [0.208511, 0.36016225],
                [0.50005719, 0.15116629],
                [0.07337795, 0.5461693],
                [0.59313011, 0.67278036],
                [0.95355644, 0.04595797],
                [0.40002513, 0.86428026],
            ]
        ),
    )
...
def test_rect_gerss():
    ...
    strata = RectangularStrata(strata_number=[2, 2], random_state=1)
    x = TrueStratifiedSampling(
        distributions=marginals, strata_object=strata, nsamples_per_stratum=1
    )
    model = PythonModel(
        model_script="python_model_function.py", model_object_name="y_func"
    )
    ...
    gpr = GaussianProcessRegression(
        kernel=kernel1,
        hyperparameters=[1, 10 ** (-3), 10 ** (-2)],
        optimizer=optimizer1,
        optimizations_number=100,
        noise=False,
        regression_model=LinearRegression(),
        random_state=0,
    )
    # gpr.fit(samples=x.samples, values=rmodel.qoi_list)
    refinement = GradientEnhancedRefinement(
        strata=x.strata_object,
        runmodel_object=rmodel,
        surrogate=gpr,
        nearest_points_number=4,
    )
    z = RefinedStratifiedSampling(
        stratified_sampling=x, random_state=2, refinement_algorithm=refinement
    )
    z.run(nsamples=6)
    assert np.allclose(
        z.samples,
...
def test_vor_rss():
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [1.78345908, 0.01640854],
                [1.46201137, 0.70862104],
                [0.4021338, 0.05290083],
                [0.1062376, 0.88958226],
                [0.61246269, 0.47160095],
                [0.85778034, 0.72123075],
            ]
        ),
    )
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [0.89172954, 0.01640854],
                [0.73100569, 0.70862104],
                [0.2010669, 0.05290083],
                [0.0531188, 0.88958226],
                [0.30623134, 0.47160095],
                [0.42889017, 0.72123075],
            ]
        ),
    )
...
def test_rss_runmodel_object():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        refinement = GradientEnhancedRefinement(
            strata=x.strata_object, runmodel_object="abc", surrogate=gpr
        )
        RefinedStratifiedSampling(
            stratified_sampling=x,
            samples_number=6,
...
def test_rss_kriging_object():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        refinement = GradientEnhancedRefinement(
            strata=x.strata_object, runmodel_object=rmodel, surrogate="abc"
        )
