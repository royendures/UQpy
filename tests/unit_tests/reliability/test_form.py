...
def setup():
    model = PythonModel(
        model_script="pfn1.py", model_object_name="model_i", delete_files=True
    )
...
def test_seeds_xu_is_none(setup):
    ...
    np.testing.assert_allclose(form_obj.u_record[0][0][0], [0.0, 0.0], rtol=1e-02)
...
def test_tol1_is_not_none(setup):
    ...
    dist = [dist1, dist2]
    form_obj = FORM(
        distributions=dist, runmodel_object=setup, seed_u=[1, 1], tolerance_u=1.0e-3
    )
    form_obj.run()
    for file_name in glob.glob("Model_Runs_*"):
        shutil.rmtree(file_name)
...
def test_tol2_is_not_none(setup):
    ...
    dist = [dist1, dist2]
    form_obj = FORM(
        distributions=dist, runmodel_object=setup, seed_u=[1, 1], tolerance_beta=1.0e-3
    )
    form_obj.run()
    for file_name in glob.glob("Model_Runs_*"):
        shutil.rmtree(file_name)
...
def test_form_example():
    ...
    os.chdir(path)
    model = PythonModel(
        model_script="pfn3.py", model_object_name="example1", delete_files=True
    )
    RunModelObject = RunModel(model=model)
    dist1 = Normal(loc=200.0, scale=20.0)
    dist2 = Normal(loc=150, scale=10.0)
    Q = FORM(
